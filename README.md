...
Files are self-contained and immutable. Bloom filters are stored in a compact binary encoding, sized from the measured distinct entries of each block and of the whole file. All of a file's block filter sections sit together in one contiguous region behind the row data, so a query consults many blocks' filters per read — a single read for any file whose region fits one 4MiB chunk.
...
The merge rewrites data block by block, with row data memory bounded per block rather than per file. Blocks that merge are decompressed and their rows re-streamed: every row feeds the rebuilt filters, and the combined rows are recompressed with the engine's current compression config (so differing compression settings are supported and consolidated). The merged block's compressed row data is buffered in memory until the block completes, because its CRC32C goes into the block metadata — bounded by `MaxRowGroupBytes`. Blocks with no merge partner are copied verbatim (after their filter-section CRC and row-data hash are verified), then re-streamed once for entry collection so the output file's rebuilt file-level filters cover them too. The output's block filter sections — rebuilt and copied alike — are buffered until its row data is complete and then written as the output's contiguous block filter region, which costs roughly 1-5% of the output size in memory for the duration of the merge. A corrupt source block fails the merge instead of propagating into the output.
...
Candidate files stream from the MetaStore iterator through a bounded pipeline: a file stage pulls one file at a time, enforces strict prefilter semantics itself (re-filtering whatever the MetaStore yields), tests the file-level bloom filters, and releases them immediately after, so per-query memory does not scale with the candidate-file count or filter size. Each surviving file is then opened once and its candidate blocks' filter sections are read in chunks of at most 4MiB — they are contiguous on disk, so one read covers many blocks, and a file whose whole region fits a chunk costs one read — and only the blocks that survive become row-scan jobs. The chunk cap bounds a file worker's transient memory regardless of how many blocks a file holds, and each chunk starts at the block being consulted, so gaps left by a prefilter are skipped rather than read. Block scans borrow handles back from a per-query, per-file pool, so a query's opens *and* its filter reads scale with its candidate files rather than with their data blocks; the pool closes every handle when the query winds down. A query without bloom conditions never touches the block filter region at all.
